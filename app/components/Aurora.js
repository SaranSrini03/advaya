...
function hexStopsToUniform(hexList) {
...
export default function Aurora({
  colorStops = ["#065f46", "#10b981", "#34d399"],
  amplitude = 1.0,
  blend = 0.5,
  speed = 1.0,
  time: timeProp,
  ...
  useEffect(() => {
    ...
    if (!renderer.isWebgl2) {
      console.warn("Aurora: WebGL2 required for aurora shader.");
      return undefined;
    }
    ...
    const initialStops = hexStopsToUniform(
      propsRef.current.colorStops ?? ["#065f46", "#10b981", "#34d399"]
    );
    ...
    const update = (t) => {
      ...
      const timeUniform = p.time !== undefined ? p.time : t * 0.01;
      if (program) {
        program.uniforms.uTime.value = timeUniform * (p.speed ?? 1) * 0.1;
        program.uniforms.uAmplitude.value = p.amplitude ?? 1.0;
        program.uniforms.uBlend.value = p.blend ?? 0.5;
        program.uniforms.uColorStops.value = hexStopsToUniform(p.colorStops ?? ["#065f46", "#10b981", "#34d399"]);
